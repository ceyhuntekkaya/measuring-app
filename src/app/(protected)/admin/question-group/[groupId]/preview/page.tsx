...
import type {QuestionDto, QuestionGroupDto} from '@/api/generated/model';
...
export default function QuestionGroupPreviewPage() {
  const params = useParams();
  const groupId = params.groupId as string;

  const {data: questionGroupData, isLoading} = useGetQuestionGroupById(groupId, {
  ...
  const sortedQuestions = useMemo(() => {
    const questions = [...(selectedQuestionGroup?.questions || [])] as QuestionDto[];
    questions.sort((a, b) => (a.orderNumber ?? 0) - (b.orderNumber ?? 0));
    return questions;
            ...
            {sortedQuestions.length === 0 ? (
              ...
              sortedQuestions.map((question: QuestionDto, questionIndex: number) => (
                ...
                >
                  ...
                  <div className="mt-4">{renderQuestionTemplate(question)}</div>
                </div>
              ))
            )}
          </div>
