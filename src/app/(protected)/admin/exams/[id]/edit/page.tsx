...
export default function ExamFormPage() {
    ...
    // Get exam by ID
    const { data, isLoading: isLoadingExam } = useGetExamById(examId, {
        query: { enabled: isEdit && !!examId }
    });
    ...
    const examLoading = isLoadingExam || isCreating || isUpdating;
    ...
    const handleSubmit = async (formData: CreateExamRequest | UpdateExamRequest) => {
        if (isEdit && examId) {
            updateExam({ id: examId, data: formData as UpdateExamRequest });
        } else {
    ...
    return (
        <div className="space-y-6">
            <PageHeader
                title={isEdit ? 'Sınav Düzenle' : 'Yeni Sınav Oluştur'}
                actions={
                    <Button
                        variant="outline"
